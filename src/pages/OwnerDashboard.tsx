...
export default function OwnerDashboard({ onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    const totalPremium = policies.reduce((s, p) => s + Number(p.premium_amount), 0);
    const totalClaims = claims.filter(c => c.status === 'Approved' || c.status === 'Settled').reduce((s, c) => s + Number(c.amount), 0);
  ...
  const quickStats = [
    ...
    { label: 'Due Renewals', value: stats.dueRenewals, icon: RefreshCw, color: 'cyan' },
        ...
        <div className="lg:col-span-8 space-y-8">
            ...
            <div className="glass dark:glass-dark border border-white/20 dark:border-white/5 rounded-[32px] overflow-hidden shadow-xl">
                ...
                <div className="divide-y divide-white/20 dark:divide-white/5">
                    {pendingList.length > 0 ? pendingList.map((customer, i) => (
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3 mt-1">
                                            <span className="text-xs font-bold text-slate-500 flex items-center gap-1">
                                                <Users className="w-3 h-3" /> {customer.assigned_to_name || 'Unassigned'}
                                            </span>
                         ...
                         <div key={claim.id} className="p-5 rounded-[24px] bg-white/40 dark:bg-white/5 border border-white/20 dark:border-white/5 flex flex-col gap-4">
                             <div className="flex items-start justify-between">
                                 <div>
                                     ...
                                     <h4 className="font-black text-slate-900 dark:text-white truncate max-w-[150px]">{claim.customer_name}</h4>
                                 </div>
                                 <span className={cn(
